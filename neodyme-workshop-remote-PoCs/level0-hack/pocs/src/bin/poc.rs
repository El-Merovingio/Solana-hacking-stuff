...
pub fn main() {
    ...
    let mut env = RemoteEnvironment::new_with_airdrop(cliente1, keypair(2), 10000000000);
            ...
            for _ in tqdm!(0..100) { thread::sleep(Duration::from_millis(30)); }
            println!("");   
            println!("");   
            env.execute_as_transaction(
                &[Instruction {
            ...
            println!("");
           /* 
            // get the Program Executable Data --->ACCOUNT<---
            let up_program = env.get_account(programa).unwrap();
            let decoded: UpgradeableLoaderState = up_program.deserialize_data().unwrap();

            println!("{} {:?} {} {:?}", "Executable Data for program: ", programa, "is: ", decoded);
            println!("");

            let deco2 = env.get_account(programa).unwrap();
            println!("DECO 2: {:?}", deco2);
             */
            ...
            //println!("Getting Program Account Data address: {:?}", env.get_account(programdata_address));

            //let jat: UpgradeableLoaderState<>::  = env.get_serde_deserialized_account(programa).unwrap();
            //println!("Serde: {:?}", upgradable);
            //println!("Program data account info: {:?}", env.get_account(decoded).unwrap());
            
            //let decoded = env.get_account(programa).unwrap().data;
           // let encoded = bs58::encode(decoded).into_string();
            //let encoded = bs58::encode(decoded).into_string();

            //println!("{} {:?}", "Program Data address: ", encoded);
